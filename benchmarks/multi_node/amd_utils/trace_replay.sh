...
# Preserve the result schema from the original AgentX PR while fixed-sequence
# and other agentic paths use the current shared aggregation module.
write_agentic_result_json() {
    ...
    (
        ...
        RESULT_DIR="$result_dir" AGENTIC_OUTPUT_DIR="${AGENTIC_OUTPUT_DIR:-$INFMAX_CONTAINER_WORKSPACE}" \
            "$AIPERF_PYTHON" -m utils.agentic.aggregation.process_agentic_result_agentic
    )
